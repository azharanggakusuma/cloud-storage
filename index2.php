...
<body class="bg-gray-100 font-sans">
  ...
  <div class="p-5 sm:p-10 md:p-16 lg:p-20 xl:p-24 hidden animate__animated animate__fadeIn animate__delay-0.5s"
    ...
    <?php
    // Nilai default foldel Tugas Kuliah
    //$isTugasKuliahAccessible = false;
    ...
    if (!$result) {
      ...
      // Perbarui nilai
      if ($status === "true") {
        $isTugasKuliahAccessible = true; // Mengubah nilai menjadi boolean true
      } else {
        $isTugasKuliahAccessible = false; // Mengubah nilai menjadi boolean false
      }
    }
    ?>
            ...
            <div class="category-container mb-4">
              ...
              <ul id="<?php echo $category; ?>" class="file-list hidden mt-2 space-y-2 pl-6">
                <?php foreach ($files as $file): ?>
                  <?php
                  $filePath = "uploads/" . $category . "/" . $file['keterangan'] . "/" . $file['filename'];
                  $isTugasKuliahFolder = ($category === 'Tugas Kuliah');
                  $isFileAccessible = ($isTugasKuliahFolder && $isTugasKuliahAccessible) || !$isTugasKuliahFolder;
                  ?>
                  <ul class="file-list-item flex justify-between items-center">
                    <div class="flex flex-col">
                      <?php if ($isFileAccessible): ?>
                    ...
                    <div class="flex">
                      <?php if ($isFileAccessible): ?>
                        <a class="file-preview ml-2 text-gray-500" href="javascript:void(0);"
                          onclick="openPreview('<?php echo $file['filename']; ?>', '<?php echo $category; ?>', '<?php echo $file['keterangan']; ?>')"
                          title="Preview"><i class="fas fa-eye"></i></a>
                        <a class="file-download ml-2 text-gray-500"
                          href="download.php?filename=<?php echo $file['filename']; ?>&category=<?php echo $category; ?>&keterangan=<?php echo $file['keterangan']; ?>"
                          title="Download"><i class="fas fa-download"></i></a>
                      <?php else: ?>
                        <span class="ml-2 text-gray-300"><i class="fas fa-eye"></i></span>
                        <span class="ml-2 text-gray-300"><i class="fas fa-download"></i></span>
                      <?php endif; ?>
                    </div>
                  </ul>
                <?php endforeach; ?>

...
<head>
  ...
  <title>User | Cloud Storage v2</title>
  <!-- Favicon -->
...
<body class="bg-gray-100 font-sans">
      ...
      <div class="container mx-auto">
        <div class="flex items-center justify-between px-4">
          ...
          <a href="auth/logout.php"
            class="mr-2 bg-red-500 text-white rounded-full py-2 px-5 inline-block hover:bg-red-600 focus:outline-none focus:ring focus:ring-red-600">
            Log Out
          </a>
        </div>
      </div>
      ...
      <nav class="bg-white border-gray-200 py-2.5 dark:bg-gray-900 shadow-sm" id="mainNav">
        <div class="container-nav flex flex-wrap items-center justify-between max-w-screen-xl px-4 mx-auto">
          ...
          <div class="flex items-center lg:order-2">
            <a href="auth/logout.php"
              class="hidden sm:flex bg-red-500 text-white dark:text-white hover:bg-red-600 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-xs px-3 lg:px-4 py-1.5 lg:py-2 sm:mr-2 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800">
              Log Out
            </a>
          ...
          <div class="items-center justify-between hidden w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
            <ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0 lg:mr-20">
              ...
              <li>
                <a href="#about-me"
                  class="scroll-link block py-2 pl-3 pr-4 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-blue-500 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700">About</a>
              </li>
              <li>
                <a href="#service"
              ...
              <li>
                <a href="auth/logout.php"
                  class="md:hidden lg:hidden ml-3 mt-2 mb-2 sm:ml-0 sm:flex bg-red-500 text-white dark:text-white hover:bg-red-600 focus:ring-4 focus:ring-gray-300 font-semibold rounded-lg text-md px-5 block text-center lg:px-4 py-2 lg:py-2 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800">
                  Log Out
                </a>
              </li>
            </ul>
  ...
  <div class="p-5 sm:p-10 md:p-16 lg:p-20 xl:p-24 hidden animate__animated animate__fadeIn animate__delay-0.5s"
    ...
    <div class="container mx-auto px-4 mt-10 mb-10">
      <div id="message"
        ...
        <div class="flex flex-col md:flex-row items-center mb-6">
          ...
          <h1 class="text-xl md:text-2xl font-semibold text-white">Halo, Selamat Datang! <span class="font-bold"></span>
            <?php echo $username; ?></span>
          </h1>
        </div>
        <p class="text-sm text-white">Kami senang Anda memilih layanan penyimpanan awan (cloud storage) kami.</p>
    ...
    <div class="bg-white p-6 rounded shadow-md mb-8">
      <h2 class="text-2xl mb-4 font-bold">Unggah File</h2>
      <form action="upload.php" method="post" enctype="multipart/form-data" class="space-y-4">
        <div>
          <label class="text-gray-700 font-semibold">Pilih File</label>
          <input type="file" name="fileToUpload" id="fileToUpload" class="border w-full p-2 mt-1">
        </div>
        <div>
          <label class="text-gray-700 font-semibold">Pilih Kategori</label>
          <select name="category" class="border w-full p-2 mt-1">
        ...
        <div>
          <label class="text-gray-700 font-semibold">Keterangan / Sub Folder</label>
          <input type="text" name="keterangan" class="border w-full p-2 mt-1"
            placeholder="Ketikkan keterangan atau sub folder">
        </div>
        <button
          class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
          type="submit" name="submit">Unggah</button>
      </form>
    </div>
    ...
    if (!$result) {
      ...
      // Perbarui nilai
      if ($status === "true") {
        $isTugasKuliahAccessible = true; // Folder "Tugas Kuliah" dapat diakses
      } else {
        $isTugasKuliahAccessible = false; // Folder "Tugas Kuliah" tidak dapat diakses
      }
    }
    ?>
        ...
        <div class="w-full md:w-1/2">
          <?php foreach ($filesByCategory as $category => $files): ?>
            <div class="category-container mb-4">
              <div
                class="flex items-center cursor-pointer <?php echo ($category === 'Tugas Kuliah' && !$isTugasKuliahAccessible) ? 'disabled-folder' : ''; ?>"
                onclick="toggleCategory('<?php echo $category; ?>')">
                <i
                  class="fas fa-folder text-<?php echo ($category === 'Tugas Kuliah' && !$isTugasKuliahAccessible) ? 'gray' : 'blue'; ?>-500 mr-2"></i>
                <h3
                  class="category-title text-md font-semibold <?php echo ($category === 'Tugas Kuliah' && !$isTugasKuliahAccessible) ? 'text-gray-500' : ''; ?>">
                  <?php echo $category; ?>
                  <?php if ($category === 'Tugas Kuliah' && !$isTugasKuliahAccessible): ?>
                    <span class="text-red-600">(Ditutup Sementara)</span>
                  <?php endif; ?>
                </h3>
              </div>
              <?php if ($category !== 'Tugas Kuliah' || $isTugasKuliahAccessible): ?>
                    ...
                    <ul id="<?php echo $category . '_' . $keterangan; ?>" class="file-list hidden mt-2 space-y-2 pl-6">
                      <?php foreach ($files as $file) {
                        if ($file['keterangan'] === $keterangan) {
                          ...
                          <li class="file-list-item flex justify-between items-center">
                            <div class="flex flex-col">
                              <a class="file-link text-blue-500 hover:underline" href="<?php echo $filePath; ?>" target="_blank">
